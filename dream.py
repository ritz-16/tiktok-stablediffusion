...
username=''
event=''
mydb=mysql.connector.connect(host='localhost',user='root',password='@Padartha1',database='tiktoksd')#mysql database connector
...
def main():
    ...
    global counter#to increment the global counter
    sql1=('use tiktoksd')
    global mydb
    cursor=mydb.cursor()
    cursor.execute(sql1)
    data2=(counter,)
    sql2=('select * from data1 where id=%s')
            ...
            def image_writer(image, seed, upscaled=False):
                ...
                if do_grid:
                    ...
                    if upscaled and opt.save_original:
                        ...
                        x=mydb.cursor()
                        sqlcheck=(f'select imagePath from data1 where id={counter}')
                        x.execute(sqlcheck)
                        fetched =x.fetchone()
                        filename=fetched[0]
                        sqlupdate=('update data1 set updatedAt=%s where id=%s')
                        dataupdate=(datetime.datetime.now(),counter)
                        x.execute(sqlupdate,dataupdate)
                        mydb.commit()
